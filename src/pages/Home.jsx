...
// This function highlights matching search text inside dashboard content.
...
const Home = ({ highlight }) => {
  ...
  // This function submits the entered lecture code to the backend and records attendance on success.
  const handleCheckIn = async () => {
    const trimmedCode = checkInCode.trim();
    if (!trimmedCode) {
      setCheckInError("Please enter a lecture code");
    ...
    try {
      ...
      setCheckInStatusText(data.message || "Checked in successfully");
      setCheckInError("");
      setCheckInCode("");
      setIsCheckInOpen(false);
    } catch (err) {
      setCheckInSuccess(false);
      setCheckInError("Server error. Please try again.");
    } finally {
      setIsCheckingIn(false);
    }
  };

  // This function converts a timetable time string into minutes for sorting and comparison.
  const toMins = (hhmm) => {
    ...
    >
      ...
      >
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="hidden sm:block text-3xl">Student</div>
        </div>
      </motion.div>
      ...
      >
        {[
          ...
          { color: "bg-yellow-100", title: "Courses", text: `${modules.length}`, onClick: () => setIsCoursesOpen(true) },
          {
            ...
            text: checkInSuccess ? "Checked in" : "Tap to enter your lecture code",
            onClick: () => {
              setCheckInError("");
          ...
          <motion.div
            ...
            className={`${tile.color} rounded-2xl p-4 shadow-md cursor-pointer hover:shadow-lg transition`}
          >
        ...
        {isCheckInOpen && (
          ...
          >
            ...
            >
              ...
              {checkInError && (
                <p className="text-sm text-red-600 mb-3">{checkInError}</p>
              )}
              ...
              <div className="flex justify-end space-x-3">
                ...
                >
                  {isCheckingIn ? "Submitting..." : "Submit"}
                </button>
              </div>
            </motion.div>
          </motion.div>
